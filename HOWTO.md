...
Выбранная схема отмечается галочкой (или любым другим способом, какой вам понравится). Схема считается выбравнной только в том случае если каждый цвет из схемы присутствует в наборе выбранных цветов. То есть если вы выбрали схему и удалили один цвет, то схема уже не будет считаться выбранной.
...
Страница состоит из двух частей: таблицы, в которой можно посмотреть Hex и RGB значения для каждого цвета, а также изменить названия перменных. Вторая часть отображает валидный код для выбранного препроцессора. 
